...
q   = 1.5; % LineWidth
m   = 50;  % Puntos 3D en cada rayo
z   = 12;  % altura adicional en el eje Z para poner el texto
f   = 18;  % FontSize
col = 'rgbcmyk';
...
axis([0 700 0 700 0 700]); grid on;axis off;pause
...
for j=3:m-3
    ...
    plot3(700,u2,v2,'mo','LineWidth',q)
    pause(0.1)
    disp([u2,v2])
    if ini==1
        Mini = [700,u2,v2];
...
Mend = [700,u2,v2];
plot3([Mini(1) Mend(1)],[Mini(2) Mend(2)],[Mini(3) Mend(3)],'m','LineWidth',q)
%text(700,u2,v2+z,'$\ell_{12}$','Interpreter', 'latex','FontSize',f)
text(700,50,250,'$\ell_{12}$','Interpreter', 'latex','FontSize',f)
...
for j=3:m-3
    ...
    u1 = w(1)/w(3);
    v1 = w(2)/w(3);
    plot3([350 v1],[0 700],[0 u1],'g:','LineWidth',q)
    plot3(v1,700,u1,'go','LineWidth',q)
    pause(0.1)
    disp([u2,v2])
    if ini==1
        Mini = [v1,700,u1];
...
%text(v1,700,u1+z,'$\ell_{21}$','Interpreter', 'latex','FontSize',f)
text(50,700,250,'$\ell_{21}$','Interpreter', 'latex','FontSize',f)
axis([0 700 0 700 0 700]); grid on;axis off
